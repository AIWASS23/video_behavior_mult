...
def page_cover(pdf):
    ...
    fig.text(0.5, 0.33, "Módulos implementados", ha="center",
             fontsize=8, color=ACCENT, va="top", fontweight="bold", linespacing=1.7)
    ...
    cols = 5
    bw, bh = 0.155, 0.038
    gap_x, gap_y = 0.038, 0.022
    for i, m in enumerate(modules):
        col = i % cols
        row = i // cols
        bx = 0.08 + col * (bw + gap_x)
        by = 0.268 - row * (bh + gap_y)
        colored_box(fig, bx, by, bw, bh, m, bg=BLUE if i < 8 else ACCENT, fontsize=7.5)

    fig.text(0.5, 0.175, "Implementado em Python 3.12+  com  MediaPipe · OpenCV · Matplotlib · NumPy · SciPy",
             ha="center", fontsize=8.5, color="#aaaaaa", va="top")

    fig.text(0.5, 0.110, "Marcelo de Araújo",
             ha="center", fontsize=11, color="white", fontweight="bold", va="top")
